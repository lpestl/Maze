...
1. Игрок проверяет ячейку, которая находиться перед ним:
   1. Если это стена:
      1. игрок поворачивается на 90 градусов **по часовой стрелке**
	  1. игрок повторяет _шаг 4_
   1. Если это свободная ячейка:
	  1. игрок выполняет _шаг 5_
...
1. По истории всех посещенных ранее ячеек, проверяем был игрок в этой новой ячейке или нет:
   1. Если игрок был в этой ячейке:
      1. удаляем из истории посещенных ячеек все записи, начиная с последней, до координат текущей ячейки **включительно**
	  1. переходим к _шагу 7_
